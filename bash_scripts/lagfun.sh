...
while getopts ":hps" OPT ; do
    case "${OPT}" in
      h) _usage ; exit 0 ;;
      p) ffmpeg -hide_banner -i "${2}" -c:v prores -profile:v 3 -filter_complex $filterComplex -map '[out]' -f matroska - | ffplay -
         ...
         ;;
      s) ffmpeg -hide_banner -i "${2}" -c:v prores -profile:v 3 -filter_complex $filterComplex -map '[out]' "${2%.*}_lagfun.mov"
